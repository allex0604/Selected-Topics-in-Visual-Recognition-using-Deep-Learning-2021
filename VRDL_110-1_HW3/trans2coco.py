        ...
        for color, sub_mask in sub_masks.items():
            ...
            for i in range(len(polygons)):
                ...
                segmentation = [np.array(
                                        polygons[i].exterior.coords
                                        ).ravel().tolist()]

                annotation = create_annotation_format(polygons[i],
                                                      segmentation,
                                                      image_id,
                                                      category_id,
                                                      annotation_id)
...
if __name__ == "__main__":
    ...
    for img_name in os.listdir(p):
        ...
        a, b, c = images_annotations_info(train_path,
                                          mask_path,
                                          img_name,
                                          image_id)
        coco_format["images"] = a
        coco_format["annotations"] = b
        annotation_cnt = c

        with open("{}.json".format(img_name), "w") as outfile:
            json.dump(coco_format, outfile)
        print("Created %d annotations "
              "for images in folder: %s" % (annotation_cnt, mask_path))
        image_id += 1
